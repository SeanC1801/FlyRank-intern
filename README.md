...
This repository contains the backend API assignments for FlyRank.
...
## [BE-02] Connecting CRUD to the Database (`be-02-connect-database/`)

### Progress & Purpose
- Replaced the in-memory CRUD operations with an SQLite database (`tasks.db`) to store tasks persistently.
- Implemented CRUD (Create, Read, Update, Delete) endpoints with `sqlite3`.
...
The database file lives at `be-02-connect-database/tasks.db`. It is created automatically the first time the application runs if it doesn't already exist. We `.gitignore` this file so each clone of the repository gets a fresh database.
